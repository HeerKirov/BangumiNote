 ...
 */
open class ConstProxy(private val appName: String = "", private val init: (ConstProxy.()->Unit)?=null) {
    //添加一条常规条目。
    ...
    fun url(key: String, url: (Map<String, Any?>)->String) {
        lists.put(key, ProxyURL(appName, url))
    }
    //查询所需要的条目。
...
class ProxyURL(private val appName: String, private val build: (Map<String, Any?>)->String) : ProxyItem {
    companion object {
        private val empty = HashMap<String, Any?>()
    }
    override fun getValue(params: Map<String, Any?>?): Any? {
        return appName + build(params?:empty)
    }
}
